...
for file in os.listdir(path):
    if file[-2:] == '.p':
    	with open(path+file, 'rb') as f:
    		...
    		epoch_skip = data['epochs_per_result']
    		iterations = len(data['clone_epoch'])
    		all_x = [i * epoch_skip for i in range(iterations)]
    		...
    		clone_means = [np.mean(epoch) for epoch in data['clone_epoch']]
    		clone_std = [np.std(epoch) for epoch in data['clone_epoch']]
    		...
    		plt.savefig('results/cloning_graphs/{}.png'.format(model_name))
    		plt.gcf().clear()